% This script is intended to introduce you how can you create different 
% type of variables and assign values to them in Matlab.
%% Scalars
% In Matlab the default variable type is the double.
doubleScalar = 6.4; % 64 bit scalar variable (double)
singleScalar = single(3.2); % 32 bit scalar variable (single)
integerScalar = int8(100); % 8 bit scalar (int8)
logicalVariable = boolean(1); % logical varialbe (boolean: true/false)
logicalVariableOtherForm = true;
...
% In Matlab there are two variable types to store texts: the character
% vectors and the strings. The different types support different
% operations.
characterVector = 'This is a character vector';
matlabString = "This is a string";